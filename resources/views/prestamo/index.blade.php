    ...
    <table id="tableH" class="table table-hover">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    @can('prestamo.edit')
                    <a href="{{ url('prestamo/'.$prestamo->id.'/edit' ) }}" class="btn btn-warning btn-sn"
                        data-toggle="modal" data-target="#ModalEdit">Devolucion</a>
                </td>
                @endcan
                <td>
                    <form action="{{ url('prestamo/'.$prestamo->id) }}" method="post" class="form__delete">
                        @method("DELETE")
                        @csrf
                        @can('prestamo.destroy')
                        <button type="submit" onclick="return confirm('¿Deseas eliminar este registro?')"
